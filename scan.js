(function () {
   ...
   function scanJobInfo(url) {
      var jobInfo = {};
      console.log("URL AND INDEX OF URL");
      console.log(url);
      console.log(url);
   ...
   function scanLinkedInPage() {
      var jobInfo = {};
      jobInfo.jobTitle = $('.jobs-top-card__job-title').text().trim();
      jobInfo.company = $('.jobs-top-card__company-info a').text().trim();
      console.log(jobInfo.company);
      if (jobInfo.company === '' || jobInfo.company === undefined) {
         jobInfo.company = $('.jobs-top-card__company-info').clone().find('span').remove().end().text().trim();
      }
      jobInfo.location = $('.jobs-top-card__bullet').first().text().trim();
